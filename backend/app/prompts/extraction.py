from app.services.llm_utils import AESTHETIC_TAGS_STR, PLACE_CATEGORIES_STR
...
Return JSON with keys: title, city, country, address, category (one of {PLACE_CATEGORIES_STR}), {_TAGS_RULE}, description, aestheticNote, confidence (0-1).
...
{_TAGS_RULE}, description, aestheticNote, confidence (0-1).
...
QUERY_EXTRACTION_PROMPT = f"""Using the user query and web search snippets below, build a travel place card.
Return JSON with keys: title, city, country, address, category, {_TAGS_RULE}, description, aestheticNote, confidence (0-1).
